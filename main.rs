...
impl ThreadPool {
    ...
    fn new(size: usize) -> ThreadPool {
        ...
        // wrap receiver so multiple workers can share it
    ...
    fn new(id: usize, receiver: Arc<Mutex<mpsc::Receiver<Message>>>) -> Worker {
        ...
        let thread = thread::spawn(move || {
            loop {
                ...
                match message {
                    Message::NewJob(job) => {
                        println!("Worker {} is processing a task", worker_id);
                        job();
                    }
                    Message::Terminate => {
                        println!("Worker {} is shutting down", worker_id);
                        break;
                    }
                }
            }
